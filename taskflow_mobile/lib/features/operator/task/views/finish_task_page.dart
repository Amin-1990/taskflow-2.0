...
class _FinishTaskPageState extends ConsumerState<FinishTaskPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_operatorController.text != state.operatorInput) {
      _operatorController.text = state.operatorInput;
      _operatorController.selection = TextSelection.fromPosition(
        TextPosition(offset: _operatorController.text.length),
      );
    }
      ...
      body: ListView(
        ...
        children: [
          const Text('Identification Operateur',
              style: TextStyle(color: Color(0xFFB8C6DE), fontSize: 16)),
            ...
            children: [
              Expanded(
                child: TextField(
                  ...
                  onChanged: notifier.setOperatorInput,
                  style:
                      const TextStyle(color: Color(0xFFE8EEF8), fontSize: 22),
                  decoration: const InputDecoration(
                    hintText: 'ID ou matricule operateur',
                  ),
                ),
              ),
              ...
              ScannerButton(
                onScan: (value) {
                  ...
                  notifier.setOperatorInput(value);
          ...
          SizedBox(
            ...
            child: FilledButton.tonalIcon(
              onPressed: state.isLoading ? null : notifier.loadCurrentTask,
              ...
              label: const Text('Rechercher la production en cours'),
            ),
          ),
          const SizedBox(height: 18),
          if (state.currentTask == null) ...[
            Text(
              state.error ?? 'Aucune tache en cours.',
            ...
            TextField(
              ...
              decoration:
                  const InputDecoration(hintText: 'Probleme machine, rebuts...'),
            ),
            if (state.error != null) ...[
              const SizedBox(height: 8),
              Text(state.error!,
                ...
                onPressed: state.isSubmitting
                    ...
                    : () async {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                              content:
                                  Text('Production cloturee avec succes.')),
                        );
