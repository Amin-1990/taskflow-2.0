...
export const TypesMachine: FunctionComponent<TypesMachineProps> = () => {
  ...
  const [editingItem, setEditingItem] = useState<TypeMachine | null>(null);
  const [showFormModal, setShowFormModal] = useState(false);
  const [typeMachineValue, setTypeMachineValue] = useState('');
  ...
  const openEditModal = (item: TypeMachine) => {
    setEditingItem(item);
    setTypeMachineValue(item.Type_machine);
    setShowFormModal(true);
  };

  const handleSave = async () => {
    const value = typeMachineValue.trim();
    if (!value) {
      showToast.error('Le nom du type de machine est requis');
      return;
    }
    ...
    try {
      const success = editingItem
        ? await updateTypeMachine(editingItem.ID, { Type_machine: value })
        : await createTypeMachine({ Type_machine: value });
  ...
  const handleConfirmDelete = async (id: number) => {
    const success = await deleteTypeMachine(id);
    if (success) {
      setDeleteId(null);
    }
  };
  ...
  const handleExportXlsx = async () => {
    try {
      setIsExporting(true);
      const response = await typesMachineApi.exportXlsx();
      const blob = response.data;
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `types_machine_${new Date().toISOString().split('T')[0]}.xlsx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      showToast.success('Types machine exportes avec succes');
    } catch (err) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const response = await typesMachineApi.getTemplateImport();
      const blob = response.data;
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'template_types_machine.xlsx';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  const handleFileChange = async (e: Event) => {
    ...
    try {
      setIsImporting(true);
      await typesMachineApi.importTypesMachine(file);
      showToast.success('Types machine importes avec succes');
  ...
  if (loading && (!typesMachine || typesMachine.length === 0)) {
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
          <p className="mt-4 text-gray-600">Chargement des types machine...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterPanel title="Recherche">
        <div className="flex space-x-2">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Rechercher un type de machine..."
              value={recherche}
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        {!typesMachine || typesMachine.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            <p>Aucun type machine trouve</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left font-semibold text-gray-700">Type de machine</th>
                  <th className="px-6 py-3 text-right font-semibold text-gray-700">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {typesMachine.map((item) => (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              {editingItem ? 'Modifier type machine' : 'Ajouter type machine'}
            </h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={typeMachineValue}
                  onChange={(e) => setTypeMachineValue((e.target as HTMLInputElement).value)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm mx-4">
            ...
            <p className="text-gray-600 mb-6">
              Etes-vous sur de vouloir supprimer ce type de machine ? Cette action est irreversible.
            </p>
